...
public class QueryController {
    ...
    private QueryResponse execute(biz.digitalindustry.db.query.QueryProvider provider, QueryCommand command) {
        try {
    ...
    private QueryResponse toQueryResponse(QueryResult result) {
        List<Map<String, biz.digitalindustry.db.server.model.Node>> rows = new ArrayList<>(result.rows().size());
        for (Map<String, QueryNode> row : result.rows()) {
            Map<String, biz.digitalindustry.db.server.model.Node> converted = new java.util.LinkedHashMap<>();
            for (Map.Entry<String, QueryNode> entry : row.entrySet()) {
                converted.put(entry.getKey(), new biz.digitalindustry.db.server.model.Node(
                        entry.getValue().id(),
